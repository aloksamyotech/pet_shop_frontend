...
const TotalGrowthBarChart = ({ isLoading }) => {
  ...
  useEffect(() => {
    const fetchSalesData = async () => {
      try {
        ...
        if (quantity.data.success && Array.isArray(quantity.data.data) && amount.data.data.length === 12) {
  ...
  }, [navType, primary200, primaryDark, secondaryMain, secondaryLight, primary, grey200, isLoading, grey500, salesData, soldQuantities, value, year, currencySymbol, t]);
          ...
          <Grid container spacing={gridSpacing}>
            <Grid item xs={12}>
              <Grid container alignItems="center" justifyContent="space-between">
                <Grid item>
                  <Typography variant="h4" sx={{ color: '#6A9C89' }}>{t("Sales Report")}</Typography>
                </Grid>
                <Grid item>
                  <Grid container spacing={2} justifyContent="flex-end">
                    <Grid item>
                      ...
                      >
                        {years.map((yearOption) => (
                          <MenuItem key={yearOption} value={yearOption} sx={{ color: '#6A9C89' }}>
                            {yearOption}
                          </MenuItem>
                        ))}
                      </TextField>
                    ...
                    <Grid item>
                      ...
                      >
                        {status.map((option) => (
                          <MenuItem key={option.value} value={option.value} sx={{ color: '#6A9C89' }}>
                            {t(option.label)}
                          </MenuItem>
                        ))}
                      </TextField>
            ...
            <Grid item xs={12}>
              <Chart
                options={getChartData(salesData, soldQuantities, value, currencySymbol, t).options}
                series={getChartData(salesData, soldQuantities, value, currencySymbol, t).series}
                type="bar"
                height={480}
              />
            </Grid>
